...
transition name, when there is one, tries three things in order:
...
2. A 5-letter fix spoken as a word -- `PANGL` → *"Pangl transition"*.
3. Anything else (a short, non-word-like ident) spelled phonetically -- *"X-ray Yankee
   Zulu transition"*.
