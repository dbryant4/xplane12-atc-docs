...
- A **Radio events** section, split out from Deviations rather than grouped by severity:
  an [emergency, lost-comms code, or hijack code](emergencies.md), and any missed
  [check-in after a handoff](controller-positions.md#handoffs) -- one line each, in
  order, naming the rule and what happened (e.g. *"00:05 EMERGENCY: pan-pan
  declared"*). A flight with none says so plainly.
